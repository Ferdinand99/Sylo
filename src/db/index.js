...
export const MIGRATIONS = [
  ...
  // GitHub → Discord: one row per (guild, repo) watch. `token` is the webhook
  // URL's path segment (identifies the row); `secret` verifies the inbound
  // payload's HMAC signature really came from GitHub for that repo. Both are
  // generated by Sylo — see src/db/githubWatches.js and
  // src/web/routes/githubWebhook.js.
  (database) => {
    database.exec(`
      ...
      CREATE UNIQUE INDEX idx_github_watches_token ON github_watches (token);
      CREATE INDEX idx_github_watches_guild ON github_watches (guild_id, created_at);
    `);
  },

  // Optional role to ping alongside a GitHub watch's posts — see
  // registerPostgresMigration(40, ...) in src/db/githubWatches.js for the
  // matching Postgres-side catch-up.
  (database) => {
    database.exec('ALTER TABLE github_watches ADD COLUMN role_id TEXT;');
  },

  // Optional changelog file to watch on a GitHub watch: when a push touches
  // this exact path, Sylo fetches it and posts just its latest `## ` entry,
  // instead of (or alongside) the generic "N commits pushed" line — see
  // registerPostgresMigration(41, ...) in src/db/githubWatches.js.
  (database) => {
