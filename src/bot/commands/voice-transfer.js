...
export const data = new SlashCommandBuilder()
  ...
  .addUserOption((o) =>
    o.setName('user').setDescription('New owner (must be in the channel)').setRequired(true)
  );
...
export async function execute(interaction) {
  ...
  if (!ctx.channel.members.has(target.id))
    return interaction.reply({ content: 'That member is not in the channel.', ...ephemeral });
  if (target.id === ctx.row.owner_id)
    return interaction.reply({ content: 'They already own it.', ...ephemeral });
