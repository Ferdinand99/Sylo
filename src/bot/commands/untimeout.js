...
export async function execute(interaction) {
  ...
  if (!target.isCommunicationDisabled()) {
    await interaction.reply({ content: `${target.user.tag} is not timed out.`, flags: MessageFlags.Ephemeral });
    return;
  }
  if (!target.moderatable) {
    await interaction.reply({ content: "⚠️ I can't modify that member (role hierarchy).", flags: MessageFlags.Ephemeral });
    return;
  }
