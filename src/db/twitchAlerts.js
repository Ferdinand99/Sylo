...
export async function announcedStreamId(guildId, login) {
  const v = await seenValue(guildId, SCOPE, login);
  return v == null ? null : decodeLiveValue(v).ref;
}

/** The announced stream id + the message we posted, or null if not tracked. */
export async function announcedPost(guildId, login) {
  const row = await seenRow(guildId, SCOPE, login);
  if (!row) return null;
...
export async function markLive(guildId, login, streamId, post = null) {
  await markSeen(guildId, SCOPE, login, encodeLiveValue(streamId, post?.channelId, post?.messageId), {
    upsert: true,
  });
}

export async function markOffline(guildId, login) {
  await forget(guildId, SCOPE, login);
}
